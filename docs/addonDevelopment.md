...
| `network.fetch` | Launcher only. Fetch a file from the internet into an importer file role, only over https, only from the exact host names listed in the manifest's `network.hosts`, with a size and time limit. The address and time of each fetch are recorded in the run manifest. |
