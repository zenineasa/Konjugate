...
| `network.fetch` | Launcher only. Fetch a file from the internet into an importer file role (or a page of text, up to 1 MB, for the window to show, such as a symbol search), only over https, only from the exact host names listed in the manifest's `network.hosts`, with a size and time limit. The address and time of each fetch are recorded in the run manifest. |
| `analysis.infer` | Launcher only. Run causal inference on a table of time series the launcher supplies and get the inferred edges back. |
...
The manifest declares `contributes.importers` (a module that turns chosen files into a model, run by the host in a worker, plus the file roles it reads and optional sample files), `contributes.scenarios` (named sets of parameter changes with a fork time, run time, optional entity choice and effect sentences), and `contributes.pages` (bundled help pages). Permissions are `data.import`, `scenario.run`, `model.open`, `results.export`, `pages.open`, `analysis.infer` and `network.fetch`. An importer file role may accept several files (`multiple`) and list several samples, and an importer may receive options from the window and return only data, as a first step before the window asks it to build the model. A scenario may also take data from the window: an intervention with the target `supplied` names either a fixed value or (with `samples: true`) a path of time and value pairs for each entity the window names, followed as a piecewise schedule, and the window may set how far ahead to run. A launcher may list `requires` (features it depends on, currently `scenarioOverrides` and `runRecord`); a version of Konjugate that does not know one refuses the launcher. When it runs a scenario, the window may pass `overrides`, `{ parameterKey: { entityName or '*' for the global entry: { value, at, duration } or null } }`, which adjust the scenario's declared interventions for parameters the importer declared live (null removes one), and `retain: false`, which releases the run's result files once summarized. The run manifest then records the importer options and the overrides, and records nothing extra when there are none. Runs that ask for a baseline of the same length at the same time share one. The host runs the importer, runs each scenario headless as a baseline and a fork of it, opens the result in the canvas as a baseline and a forked branch, and exports a folder with the results, the project and a provenance manifest. A launcher package is rejected at install if it declares a file the archive does not contain. The design, the trust model and what is not yet built are in [Launcher add-ons](proposals/launcherAddons.md).
