...
Creating a group from a selected set of nodes expands the shared definition into a complete mesh: one bidirectional edge per unordered member pair, each independently bound to its own two nodes by matching state symbol, the same auto-bind mechanism the component library's edge templates use. Adding a node to an existing group creates its edges to every current member. Detaching a node clears the group reference on its edges to the rest of the group, leaving them behind as ordinary, independently editable edges rather than deleting them.
