...
- **Computational-node SDK contract**: the C++ and Python SDKs expose a checkpointable node-provider interface with declared inputs, multiple outputs and explicit `checkpoint()`/`restore()` hooks. The engine protocol and execution plan do not launch this provider kind yet; it is the contract for the next runtime slice.
