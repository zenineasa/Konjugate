...
- **C++ build pipeline**: `ProviderRuntime` compiles inline C++ source together with the SDK header and worker wrapper on first use, using a discovered compiler (`xcrun -find clang++` plus SDK sysroot on macOS; hardcoded MSVC install paths plus a `cl.exe`/`clang++`/`g++`/`c++` probe chain on Windows, `autoDetectCppCompiler` in `src/main.mjs`; falling back to the plain `c++` name elsewhere) and caching the built executable by a hash of the source text together with the glue file's own content (so a Konjugate upgrade that changes the glue itself invalidates a stale cached artifact). A titlebar settings dialog (⚒, next to the model assistant button) lets the user override the auto-detected compiler and interpreter path per machine, with Browse/Test actions — see [Local toolchains and runtimes](#local-toolchains-and-runtimes).
...
- **Computational-node providers**: the C++ and Python SDKs expose a checkpointable node-provider interface with declared inputs, multiple outputs and explicit `checkpoint()`/`restore()` hooks. The Python worker supports node evaluation, checkpoint and restore messages; a `cpp` node provider instead runs through a dedicated in-process ABI (`konjugate/providerInProcessAbi.hpp`'s `KonjugateInProcessNodeProviderV1`, driven by `engine/src/providerInProcessNodeShim.cpp` — the node-shaped counterpart to the relationship-shaped in-process shim), with no worker-process fallback (`providerWorker.cpp` has no node-provider protocol for C++). The native engine launches this provider kind from a model, Python or C++: `executionPlan.cpp` compiles a node's `implementation` into a `NodeProviderTask`, `providerRuntime.cpp` evaluates it once per substep and requests checkpoint/restore over the matching transport, and `simulationRunner.cpp` folds its named derivative outputs into the ordinary Euler update and wires its checkpoint payload into the engine's own checkpoint/restart mechanism (`ResultCheckpoint.provider_states`, `startCheckpoint.providerStates`). The partitioned execution backend remains unimplemented for this provider kind, and there is still no editor UI for authoring a node provider of either kind (inline JSON only).

### Not yet built

- Discovery among *multiple* compatible compilers/interpreters (today it's still auto-detect-one-or-manually-override, not a list to choose from). Windows has real auto-detection (hardcoded MSVC install paths plus a probe chain, `autoDetectCppCompiler` in `src/main.mjs`) — Linux auto-detection beyond the plain `c++`/`python3` fallback is still genuinely missing.
...
- Everything scoped as a non-goal below: an editor UI for authoring computational-node providers (including an FMI-backed one), plugin-packaged (as opposed to inline) computational-node providers, partitioned-backend execution of computational-node providers, events and commands, live/remote providers, the public engine API, and multi-output relationships (other than the computational-node provider case above). General FMI import itself is no longer a non-goal -- see [FMU/FMI](#fmufmi) below. Algebraic outputs are also no longer a non-goal, for both source terms (`setsValue`) and computational-node-provider outputs (`outputs[].setsValue`) -- see the `addGradient` section above.
...
**Both directions are now implemented.** Export (a Konjugate model becomes a real FMI 2.0 Co-Simulation `.fmu`) and general import (an arbitrary installed FMU, FMI 2.0 or 3.0 Co-Simulation, becomes a node's `implementation`) are documented in full in [Code export](codeExport.md) and [Project schema](projectSchema.md) respectively; this paragraph is kept only as a pointer, not a duplicate. In brief, import resolves a `kind: "fmi"` reference to an ordinary `cpp` computational-node-provider implementation before the engine ever runs it (`src/fmiResolver.mjs`), reports platform support at resolve time (a clear error if the installed FMU has no binary for the current platform), and reports rollback capability by simply declining `checkpoint()` when the FMU itself declines `fmi2/3Get/SetFMUState` — the same "decline, don't fake" convention export already uses. Model Exchange is not hosted (Co-Simulation only, on both sides, for the same fixed-step-integrator fidelity reason export documents). The preferred communication step is always exactly one Konjugate substep -- there is no separate, larger "communication interval" concept on the import side the way there is on the export side.
...
**`addGradient`'s value is reinterpreted, not the API, when a source term is `setsValue`.** A programmable source term's `addGradient(value)` call always reports one number to the engine; by default that number is a derivative, exactly as shown above. When the owning source term's `setsValue` is `true` (see [Project schema](projectSchema.md)), the engine treats that same number as the state's *value* for this substep instead of a rate -- the SDK itself gains no new method, and a provider's own code is identical either way. Mechanically, this makes the state a genuine algebraic one (recomputed directly every substep, never integrated) rather than a derivative-based approximation, so it composes correctly with any future solver, not just today's Explicit Euler -- see `applyAlgebraicTasks()` in `engine/src/executionPlan.cpp`.

This mechanism, and the reasoning behind it, follows directly from a design question the causal-inference feature already worked through once: [its input-replay proposal](proposals/causalInferenceInputReplay.md) explicitly considered and rejected a direct "set the value" SDK primitive, for the same solver-genericity reason ("'set the value to X' has no coherent meaning [in a multi-stage integrator]... while 'here is the instantaneous rate' composes with any solver of any order"). That feature's own solution -- replaying a CSV column's recorded values via a rate that is a pure, state-*independent* function of time -- sidesteps the problem entirely rather than solving it, because the true rate is already known exactly from the data. `setsValue` needed the general case (an arbitrary expression that may depend on other live state), so it takes the direct approach the causal-inference proposal anticipated might eventually be needed: a real algebraic-state mechanism, evaluated on its own terms rather than smuggled in as a derivative. An earlier version of `setsValue` used exactly the derivative-based shortcut that proposal warned against (`(target - currentStateValue) / stepSize`, exact only under Explicit Euler's specific arithmetic) before being replaced with this one.

**The same `setsValue` flag exists per-output on a computational-node provider's `outputs[]` entries** (see [Project schema](projectSchema.md)), for exactly the case a source term's single output can't cover: a provider that owns several of a node's states at once and knows some of them algebraically rather than as rates. `NodeOutputCollector::addGradient(key, value)` gains no new method either -- an output's `setsValue` binding, known at plan-compile time, decides how the engine treats the number that key's `addGradient` call reported. This is what closes "algebraic outputs" as a non-goal for computational-node providers (see the feature list below) and, concretely, what let general FMI import (above) drop its own Euler-specific pseudo-derivative trick -- an FMU's `causality="output"` variable is a real algebraic value, not a rate, and `src/fmiResolver.mjs` now marks every generated output `setsValue: true` instead of synthesizing a feedback input port to fake the same result under Explicit Euler specifically. See `docs/codeExport.md`'s "Importing an FMU" section for that migration's own history.
...
`describe`, `initialize`, `evaluate` and `shutdown` are implemented for the stateless relationship/source-term SDK above; the computational-node provider SDK additionally implements `checkpoint`/`restore` (Python and C++, the latter in-process only). `commit` remains future work, pending a real speculative/committed evaluation distinction.
...
**Partially implemented.** C++ auto-detection has real platform-specific logic on macOS and Windows: `xcrun -find clang++` plus the Apple SDK sysroot from `xcrun --show-sdk-path` on macOS; on Windows, hardcoded MSVC install paths (Visual Studio "18" Community and 2022 BuildTools) checked for `cl.exe`, then a `clang++`/`g++`/`c++` probe chain (`autoDetectCppCompiler` in `src/main.mjs`) — falling back to the plain `c++`/`python3` names only on other platforms (Linux). A manual override does exist now: the titlebar's "Provider toolchains" dialog (⚒) lets the user set an explicit compiler/interpreter path per machine, Browse to one via a native file picker, and Test it before saving (`providerToolchainStore.mjs`, `userData/providers/toolchains.json`) — this is consumed both by the live editor's syntax check and by the actual engine run (`providers.cpp.compiler`/`providers.python.interpreter`). What's still missing is discovery *among several* installed options presented as a list, and Linux auto-detection beyond the bare fallback names. Compiled C++ providers are cached by a hash of their source together with the glue file's own content, so an unchanged provider (against an unchanged Konjugate build) is not rebuilt on every run, but the resulting artifact identity, compiler identity and build configuration are not yet recorded in the project or results.
...
4. **Done**, macOS and Windows discovery (Linux still falls back to the plain `c++` name — see [Local toolchains and runtimes](#local-toolchains-and-runtimes)). Implement compiler discovery, a Konjugate C++ wrapper, native build validation and a C++ relationship editor.
...
8. **Partially done.** Packaging an inline provider as a reusable plugin, resolving it, and executing it through the existing worker protocol all work and are tested (`scripts/packageExamples.mjs`, `src/pluginResolver.mjs`; see [Plugin development](pluginDevelopment.md)). Not done: generating a parameter/binding configuration UI from an installed plugin's manifest without provider-specific host code — the editor still only offers Equation/C++/Python inline authoring.
...
10. **Partially done**, serial/thread-pool execution only. Stateful computational-node providers execute through the same worker protocol as relationship providers when Python, or in-process (no worker fallback) when C++, with checkpoint/restore wired into the engine's own checkpoint and restart mechanism either way. The partitioned execution backend and external-software plugins remain separate later phases.
