...
- Everything scoped as a non-goal below: an editor UI for authoring computational-node providers, plugin-packaged (as opposed to inline) computational-node providers, partitioned-backend execution of computational-node providers, algebraic outputs, events and commands, FMU/live/remote providers, the public engine API, and multi-output relationships (other than the computational-node provider case above).
