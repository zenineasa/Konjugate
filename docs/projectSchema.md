...
Projects may contain named `runConfigurations` and an `activeRunConfigurationId`. A run configuration contains `duration`, `globalTimeStep`, and `outputInterval` in seconds; output interval is an integer multiple of the global timestep. During each global step, every node advances with `globalTimeStep / substepsPerGlobalStep`. References to the advancing node use its latest local state; references to other nodes use the frozen state snapshot from the start of the global step. Nodes synchronize at the global boundary.
