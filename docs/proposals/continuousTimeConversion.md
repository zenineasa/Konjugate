...
The one real limitation, and it's honest rather than silent: this rate is calibrated to *one* Euler step at the CSV's own Δt. Run Konjugate with a much finer step size and repeated application converges toward `e^(coefficient−1)·x` rather than `coefficient·x` -- and that drift is **substantial**, not negligible: measured on the same 8-node system (roots held to their true historical trajectory, so the downstream chain has a real signal to track for the whole horizon, not a degenerate free-run that decays to zero and stays there), error grows to roughly **6-7x** the calibrated-resolution result once substeps are refined well past 1 per row -- most of the increase already present at just 2 substeps, then flattening out. This isn't a solver-accuracy gap: any convergent method (Euler, RK4, adaptive) converges to the same limit, `exp(A·Δt)`, as step size shrinks, and `A` here was derived to match `Φ` via Euler's own one-step formula, not via the exact exponential -- a higher-order solver would reach the same drifted asymptote faster, not a smaller one. What does hold: the drift **converges to a bounded multiple rather than diverging** -- a real degradation, not a cliff, and nothing like matrix log's risk of a plausible-looking wrong answer with no error at all. See `tests/engine/continuousTimeDrift.mjs` for the permanent regression guard on the bounded-not-unbounded property.
