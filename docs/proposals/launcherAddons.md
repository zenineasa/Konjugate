...
**Status: partly built.** Part 1 (plugin-contributed examples) and a first vertical slice of Parts 2, 4 and 5 (the launcher kind, an importer, scenarios, open-in-canvas, and an export with a run manifest) are implemented; see "As built" below for where the implementation departs from the text that follows. Part 3 (suites) is not built. It has been revised once after thinking through how an institutional user (an analyst, a model validator, the IT and security team that must approve the software) would meet a domain package, and the revision changed its priorities. It marks where it is still unsure.
...
The first slice was built end to end so a real package could be shown to the people it is for, and a few decisions were made along the way that differ from the design above.

- **The launcher owns its importer.** The proposal has plugins contribute importers. In the slice, `contributes.importers` lives in the launcher's own manifest, with the importer module, its bundle and node definitions, sample files and help pages all inside one add-on package. The models an importer builds are ordinary equation-based models with no dependency on any plugin, so nothing needs resolving across packages. Plugin-contributed importers, and the suite that would ship both, remain the intended direction.
- **Two-step import.** The window names a declared file role and the host shows the native picker; a separate action runs the importer over everything chosen. The importer runs in a worker thread (30 second limit, 512 MB) and receives only the file texts and helpers to parse an equation and read JSON inside its own package. The report carries errors and warnings against file, spreadsheet row and column, and a model is built only when there are no errors.
- **Scenarios run headless.** `runScenario` runs the baseline once (cached) and then a fork of it from the scenario's time, using the same engine calls the canvas uses: start the child from a checkpoint paused, apply each change as a scheduled step or pulse, then run. It returns downsampled series for the state symbols the window asks for. A scenario intervention can carry a delay (`at`) and a `duration`, which makes it a pulse that returns to the parameter's base value; a runaway that never ends is rarely the question a stress test asks. The window still cannot start anything by itself: the user presses Run, and the runs are the host's, bounded by the scenario's declared run time.
- **Interventions find parameters through a parameter index.** The importer returns, beside the model, an index of the live shared parameters it created (a key, an entity such as an institution, the shared parameter's id, and its range). A scenario names a key and a target (`chosen` entity, `all` entities, or `global`), so scenarios are data that stay valid for any model an importer builds.
- **Open in canvas carries the results.** The host builds a project bundle whose result branches are the baseline and the forked scenario, decodes it exactly as it does for an example, and sends it to the project window, so the canvas opens with the model, both runs, and the branch tree, behind the usual unsaved-work confirmation.
- **Export writes a folder,** not a single file: the results in long format (`results.csv`), a summary the window supplied, the project with its results (`project.kjt`), and `run-manifest.json` with the application and package versions, the SHA-256 of each input file and of the model, the run configuration, the scenario and its resolved changes, and the hash of every file written. The manifest records hashes of the inputs, never their contents.
- **Packaging check.** A launcher package is rejected at install if its manifest declares an entry, importer, sample or help page the archive does not contain.
- **Not yet done:** running a packaged (asar) build has not been tested with the importer worker; the suite package; a per-package "skip the confirmations" setting; and reading model state back into the window for a more responsive guide.
