...
The first protocol version is defined using Protobuf Edition 2024 in `protocol/engineProtocol.proto`. The C++ engine uses generated Protobuf bindings. Electron now uses two different decode strategies, not one: one-shot report messages (`ValidationReport`, `InferenceReport`, `FittingReport`, `InspectionReport`) go through `protobufjs` and a `pbjs`-generated static module (`scripts/generateReportProtocol.mjs` produces `src/generated/reportMessages.mjs`; `src/reportProtocol.mjs` just calls `.decode()`/`.toObject()` against it, fully generic over the message shape). The live/streaming messages (`ResultFile`, `EngineEvent`, `EngineCommand`) still go through `src/engineProtocol.mjs`'s own hand-rolled minimal wire-format decoder, kept from the initial vertical slice specifically for those message shapes -- avoiding a second runtime dependency was the original reason, but that reasoning no longer applies project-wide now that `protobufjs` is already a real dependency for the report messages; a future cleanup could migrate `engineProtocol.mjs` onto the same generated module instead of maintaining two decode strategies. Edition 2024 requires a Protobuf toolchain that supports editions; the repository's pinned vcpkg dependency and development setup provide that toolchain.

Protocol messages have grown beyond the live/result path since this document was first written: `ValidationIssueReport`'s `IssueLocationReport`/`IssueAttributedParameterReport` carry structured, precise navigation targets for a validation warning (see `engine/src/modelValidator.cpp`'s `numericsPotentiallyUnstable` warning for the producer, and `navigateToValidationIssue` in `src/renderer/renderer.mjs` for the consumer), and `ResultFile.stability_findings` (a `repeated StabilityFindingReport`) carries during-run stability-monitor findings alongside the samples/checkpoints described below -- see [Embedded binary result storage](resultFileFormat.md).
