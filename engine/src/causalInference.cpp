...
InferenceResult inferGraph(const InferenceSeries& series, const InferenceConfig& config) {
    ...
    // target's own lagged value as a control, so a source's correlation with the target's own
    // persistence is not mistaken for its effect) at a single lag chosen, together with the
    // ridge penalty, by held-out validation loss.
    for (std::size_t target = 0; target < variableCount; ++target) {
        ...
        for (const int lag : config.candidateLags) {
            ...
            const std::size_t featureCount = sources.size() + 1; // + the target's own self-lag control term
            ...
            for (std::size_t row = 0; row < usableRows; ++row) {
                ...
                for (std::size_t column = 0; column < sources.size(); ++column) {
                    x(eigenRow, static_cast<Eigen::Index>(column)) =
                        standardized.values(eigenRow, static_cast<Eigen::Index>(sources[column]));
                }
                x(eigenRow, static_cast<Eigen::Index>(sources.size())) =
            ...
            for (const double penalty : config.ridgePenalties) {
                ...
                const double score = heldOutScore(fit, xValidation, yValidation);
                if (score > bestScore) {
                    bestScore = score;
                    bestLag = lag;
                    bestFit = fit;
        ...
        // A non-positive score means this target's joint fit does not even beat predicting its
        // own held-out mean -- every one of its coefficients is fitted to noise at that point,
        // no matter how large any individual one looks, so nothing is extracted from it. Without
        // this gate, a target whose own dynamics are already well explained by its self-lag
        // term can still show a small but nonzero *standardized* coefficient on an unrelated
        // source purely from finite-sample overfitting under a weak ridge penalty, even though
        // the joint fit is clearly worse than the null model.
        ...
        // The joint intercept, de-standardized, covers every accepted source (and the discarded
        // self-lag control term) together; split evenly across this target's accepted edges so
        // their sum reconstructs it exactly, matching how Konjugate sums every edge's
        // contribution into the same derivative (projectSchema.md: "Multiple relationships
        // targeting the same state contribute additively").
        double jointInterceptOriginal = muTarget + sigmaTarget * bestFit.intercept;
        for (std::size_t column = 0; column < sources.size(); ++column) {
            const double coefficient = bestFit.coefficients(static_cast<Eigen::Index>(column));
            ...
            jointInterceptOriginal -= (coefficient * sigmaTarget / sourceSigma) * sourceMu;
        }
        const double selfLagCoefficient = bestFit.coefficients(static_cast<Eigen::Index>(sources.size()));
        jointInterceptOriginal -= selfLagCoefficient * muTarget;

        for (std::size_t column = 0; column < sources.size(); ++column) {
            const double standardizedCoefficient = bestFit.coefficients(static_cast<Eigen::Index>(column));
            if (std::abs(standardizedCoefficient) < config.coefficientThreshold) continue;
            ...
            edge.lag = bestLag;
            edge.coefficient = standardizedCoefficient * sigmaTarget / sigmaSource;
            edge.intercept = jointInterceptOriginal / static_cast<double>(sources.size());
            edge.score = bestScore;
    ...
    // ordinary lagged/dynamical edge downstream.
    for (std::size_t i = 0; i < variableCount; ++i) {
        for (std::size_t j = i + 1; j < variableCount; ++j) {
            ...
            for (const auto& [target, source] : std::vector<std::pair<std::size_t, std::size_t>>{{i, j}, {j, i}}) {
                ...
                edge.targetColumn = series.columnNames[target];
                edge.lag = 0;
                edge.coefficient = coefficientOriginal;
