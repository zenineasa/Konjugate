...
test('regenerates every entity ID and equation binding while retaining layout', () => {
    ...
    assert.deepEqual(pasted.nodes.map((node) => node.id), [20, 22]);
    assert.deepEqual(pasted.nodes.map((node) => node.position), [[1, 0, 2], [3, 0, 2]]);
    assert.deepEqual(pasted.nodes.flatMap((node) => node.states.map((state) => state.id)), [21, 23]);
    assert.equal(pasted.edges[0].id, 24);
    assert.deepEqual(pasted.edges[0].source, { nodeId: 20, stateId: 21 });
    assert.deepEqual(pasted.edges[0].target, { nodeId: 22, stateId: 23 });
    assert.equal(pasted.edges[0].parameters[0].id, 25);
    assert.deepEqual(pasted.edges[0].equationModel.bindings.map((binding) => (
        binding.nodeId ?? binding.stateId ?? binding.parameterId
    )), [20, 22, 25]);
    assert.equal(pasted.edges[0].equationModel.output.stateId, 23);
    assert.equal(pasted.nextId, 26);
});
